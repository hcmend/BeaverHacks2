...
function add_or_subtract(expression) {
    ...
    expression = expression.slice(0, matchIndex) + result + 
            expression.slice(matchIndex + fullMatch.length);
...
function while_parentheses(expression, show_work) {
    ...
    while (open_parenthesis !== -1) {
        ...
        while (sub_expression.includes('^')) {
            ...
            if (show_work) {
                print_expression(expression); // Print the entire expression
            }
        }
        ...
        // Replace the sub-expression in the original expression with parentheses
        expression = expression.slice(0, open_parenthesis) + sub_expression + expression.slice(close_parenthesis + 1);

        // Print the updated expression after replacing the sub-expression
        if (show_work) {
